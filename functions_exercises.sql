...
SELECT first_name, last_name, datediff(curdate(), hire_date) AS DAYS
FROM employees
WHERE birth_date LIKE '%12-25' AND hire_date LIKE '199%';

/* 5 Find the smallest and largest current salary from the salaries table.
	The largest salary is 158,220.
    The smallest salary is 38,623.			*/

SELECT MAX(salary), MIN(salary)
FROM salaries;

#CORRECTION for current salary
SELECT MAX(salary), 
	MIN(salary)
FROM salaries
...
the first 4 characters of the employees last name, an underscore, the month the employee was born, and the 
last two digits of the year that they were born. Below is an example of what the first 10 rows will look like:
